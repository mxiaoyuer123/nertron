...
class DomainService {
  static const String ossDomain = 'https://nertron.metatai.xyz/domain.json';
...
// 从返回的 JSON 中挑选一个可以正常访问的域名
  ...
  static Future<bool> _checkDomainAccessibility(String domain) async {
    try {
      final response = await http.get(Uri.parse('$domain/waf/guest/comm/config')).timeout(const Duration(seconds: 15));
